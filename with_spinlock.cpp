...
class LockingTree
{
    ...
    vector<Spinlock*> lockPath(vector<Node *> &path)
    {
        vector<Spinlock*> guards;
        guards.reserve(path.size());
        for(Node *n: path)
        {
            n->lock.lock();
            guards.push_back(&n->lock);
        }
        return guards;
    }
    
    void unlockPath(vector<Spinlock*> &guards)
    {
    ...
    bool lock(string val, int id)
    {
        ...
        vector<Spinlock*> guards = lockPath(path);
